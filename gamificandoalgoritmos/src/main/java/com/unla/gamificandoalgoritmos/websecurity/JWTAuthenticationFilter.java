...
public class JWTAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
	...
	//genero el token, con la fecha de creacion dato de, username, fecha expiracion, firma en hs512 de la clave secreta, y por ultimo agrego al header el token
	@Override
	protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
		...
		String token = Jwts.builder().setIssuedAt(new Date()).setIssuer(Constante.ISSUER_INFO)
				...
				.signWith(SignatureAlgorithm.HS512,Constante.SECRET_KEY).compact();
		response.addHeader(Constante.HEADER_AUTHORIZATION_KEY, Constante.TOKEN_BEARER_PREFIX + " " + token);
	}
}